...
connector `init-feature-registry`'s Step 7b calls after writing a `Feat-NNNN-{feature-id}/Index.md`
that traces back to a `DEC-NNNN` with a non-null `feature:` field. It doesn't live inside either
...
- **Never infer a match from naming alone** (e.g. a FEAT and FR sharing a similar title) — the
  ...
  behavior, every time.
- This is advisory, not a gate — the caller (`init-feature-registry` Step 7b) always writes the
