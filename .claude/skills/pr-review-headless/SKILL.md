...
- Root `CLAUDE.md`
- `your-backend-repo/CLAUDE.md` and `your-frontend-repo/CLAUDE.md` (if present)
...
- **Auth error (401/403)** → this is a hard failure. Emit the escalation JSON (Section 7) with
  `reason: "gh auth error - <details>"`.
- **PR closed or merged** → emit the escalation JSON with `reason: "PR is closed/merged, cannot review"`.
- **General API failure** → retry once; if it fails again, emit the escalation JSON with the error details.
