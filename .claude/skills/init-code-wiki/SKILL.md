...
- exactly what's about to be committed (`code-wiki/{project-id}/SCHEMA.md` if newly written, plus
  the rest of the `code-wiki/{project-id}/` tree, and `.claude/rules/`, `.claude/agents/`,
  `.claude/skills/` if Flight Deck's onboarding put them there — see Step 7)
...
# directly, and a `git add` on a missing pathspec aborts the commit. Named
