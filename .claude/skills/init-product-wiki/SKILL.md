...
- exactly what's about to be committed (`wiki/SCHEMA.md` if newly written, the
  `wiki/{project-id}/` tree, and `.claude/rules/`, `.claude/agents/`, `.claude/skills/` if Flight
  Deck's onboarding put them there and `/init-code-wiki` did not already stage them — see Step 10)
...
# Flight Deck's onboarding copies its skills, review rules and agent definitions
# into the clone before this runs, for the project to keep once this PR merges.
# Guarded because the directories are absent when a human runs this skill
# directly, and a `git add` on a missing pathspec aborts the commit. Staged by
# `/init-code-wiki` too — whichever of the two reaches its commit step first
# carries them, and the second then finds nothing new to stage. Named
# individually rather than `git add .claude`, so that anything written under
# .claude during this run — `settings.local.json`, say — stays out of the PR.
if [ -d .claude/rules ]; then git add .claude/rules; fi
if [ -d .claude/agents ]; then git add .claude/agents; fi
if [ -d .claude/skills ]; then git add .claude/skills; fi
...
- **If none is returned** — this skill is the first of the two to reach this point. Create it:
  ...
  - Seeds .claude/rules/, .claude/agents/ and .claude/skills/ if Flight Deck's onboarding supplied them (existing files left untouched)
