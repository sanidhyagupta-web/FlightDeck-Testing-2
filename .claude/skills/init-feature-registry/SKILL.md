...
### Assign `Feat-NNNN` numbers

```bash
find code-wiki/{project-id}/Features -maxdepth 1 -type d -name "Feat-*" 2>/dev/null
```

Match each feature discovered in Steps 2–4 (by service/feature name) against an existing
`Feat-NNNN-{feature-id}/` directory — a match keeps its number forever, never renumbered. A
feature with no match gets the next unused number (current max across existing directories + 1).

### Write per-feature pages

For each feature, create or update `code-wiki/{project-id}/Features/Feat-NNNN-{feature-id}/Index.md`
per the sections `code-wiki/{project-id}/SCHEMA.md` defines:
...
For each `Feat-NNNN-{feature-id}/Index.md` just written, find the decision(s) that drove it:
...
ask — most bug fixes and tech debt land here. Leave `Feat-NNNN-{feature-id}/Index.md` without a
...
**`Feat-NNNN-{feature-id}/Index.md`** — add (or update) a `## Relationships` section:
```markdown
## Relationships
...
```yaml
implemented_by: Feat-NNNN-{feature-id}
```
...
|--------|----------------|---------|--------------|-------------------|
| `{table}` | [Feat-NNNN-{feature-id}](../Features/Feat-NNNN-{feature-id}/Index.md) | {name:type, ...} | {unique/check/FK} | {table.col → other_table.col (owned by Feat-MMMM)} |
```
...
- `Feat-NNNN` numbers are permanent once assigned — match by feature/service name against existing `Feat-NNNN-{feature-id}/` directories before assigning a new number, never renumber an existing one
...
- The bridge check is advisory, never a gate — a `partial`/`unmatched` `wiki-bridge-verifier` verdict still gets written into both `Feat-NNNN-{feature-id}/Index.md` and the decision's `implemented_by` field, and is called out in Step 9's report; it never blocks the code-wiki commit or the rest of this run
