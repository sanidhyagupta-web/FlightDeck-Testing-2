...
| `UPDATE_FEATURE_REGISTRY` | init-feature-registry | Yes — flows into code wiki |
...
**Goal**: Refresh the feature registry (and its code-wiki mirror) so both always reflect the
current codebase, not just this ticket's diff.

1. Follow the `init-feature-registry` skill instructions **in full**, including its own Step 7b
   (code-wiki regeneration) — a full rescan every ticket, not an incremental diff-based update.
   `/update-feature-registry` is retired from this workflow; do not invoke it here or anywhere
   else in `/implement`.
...
1. `init-feature-registry`'s own Step 7b already regenerated every code-wiki file
   (`SCHEMA.md`, `Architecture/Overview.md`, `Schemas/schemas.md`,
   `Features/Feat-NNNN-*/Index.md`) as part of the previous phase's full rescan, so there is
   nothing left for this phase to do. It's kept as a phase name only so
   `execution-state.md` files already mid-flight from before this change still resolve to a
   known phase; do not follow `update-code-wiki`'s own instructions here — that skill expects
   `feature-registry-manifest.json`, which `init-feature-registry` never writes.
