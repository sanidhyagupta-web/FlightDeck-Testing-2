...
description: Review a frontend pull request with full context gathering, rule-based analysis, and inline comment posting.
...
Full-workflow review for the frontend repository with context gathering, configurable rules, severity-based analysis, user checkpoint, and inline comment posting.
...
- If only a number is given, default to the frontend repo <!-- CUSTOMIZE: your default frontend repo name and org -->
...
- Root `CLAUDE.md`
- `your-frontend-repo/CLAUDE.md` (if accessible)
...
- `.claude/rules/frontend-lang.md` — components, test identifiers, path aliases
- `.claude/rules/frontend-state.md` — state management, API layer, caching
- `.claude/rules/security.md` — XSS, secrets, input validation
- `.claude/rules/general-quality.md` — error handling, naming, dead code, API contracts
- `.claude/rules/testing-standards.md` — component tests, snapshot tests, assertions
...
- **Missing tests** — Flag new components, hooks, or utility functions that lack corresponding test coverage.
- **Missing config** — Flag new environment variables, route definitions, or shared type exports used without being defined in config files.
- **Cross-repo notes** — If the PR consumes new or changed API endpoints, add an informational note that backend changes may be required or in-flight.

### Quick-Reference Checklist

<!-- CUSTOMIZE: Replace checklist items with your frontend's conventions -->
...
- [ ] Uses project path aliases for shared imports (no deep relative imports)
- [ ] No raw HTML injection without sanitization
- [ ] Loading, error, and empty states handled for data-fetching components
- [ ] Accessibility: semantic HTML, labels on inputs, alt text on images
...
<missing tests, missing config, cross-repo notes — if any>
...
```
:red_circle: [FE-02] Missing data-testid on interactive button element
:yellow_circle: [GEN-08] Potential undefined access on `response.data.items` — add optional chaining
:green_circle: [GEN-03] Consider renaming `val` to `selectedValue` for clarity
:clap: Clean hook extraction — good separation of data fetching from UI
```

Use the rule ID (e.g., FE-01, GEN-08, SEC-06) in the comment for traceability.
