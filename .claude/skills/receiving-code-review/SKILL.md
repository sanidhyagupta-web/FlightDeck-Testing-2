...
- Load the relevant sub-repo CLAUDE.md (e.g. `<your-backend-repo>/CLAUDE.md` or `<your-frontend-repo>/CLAUDE.md`) for architecture, patterns, and constraints
...
**If the feedback conflicts with project conventions:** Explain the convention with a reference to the CLAUDE.md or rule file. Example: "Our integration test pattern uses real database containers rather than mocked repositories — see `<your-backend-repo>/CLAUDE.md`. I've kept the test as-is."
